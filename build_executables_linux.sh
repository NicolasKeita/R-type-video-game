...
conan install ../ --build=missing -s compiler=gcc -s compiler.version="$(gcc -v 2>&1 | tail -1 | awk '{print $3}' | cut -d "." --fields=1)" -s compiler.libcxx=libstdc++11 -s build_type=Release -g virtualenv
